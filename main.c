...
{
    int n,i, j ;
    float   h, u,x_0, p, c, y_0, k, l, fact, t;
   ...
   k=0;
   l=1;
   for(j=1; j<=n; j++)             //calculating the numerator and denominator from 1st term
   {
       fact=1;
       for(i=1; i<=j; i++)
       {
          fact=fact*i;                                 //calculating i!
       ...
       k=k+(l*dy[j][0])/fact;
                                                    ...
                                                    // printf("the value of k is %f for %d \n", k,j);
